// ==UserScript==
// @name         Zagadka discord v5.2 [Neon UI + Hub Compatible]
// @namespace    http://tampermonkey.net/
// @version      5.2
// @description  Wykrywa zagadkę, stylizowany panel Neon UI, Drag&Pin, Webhook, Kompatybilny z Bocik Hub
...
(function() {
    ...
    style.innerHTML = `
        .bocik-panel {
            position: fixed;
            bottom: 200px; right: 200px; /* Domyślna pozycja */
            width: 180px;
            ...
            color: #fff;
            overflow: hidden;
            display: none; /* Domyślnie ukryte, JS steruje */
    ...
    panel.innerHTML = `
        ...
        <div class="bocik-content">
            ...
            <button class="settings-btn" id="toggleConfig">⚙️ Ustawienia</button>

            <div class="config-container" id="configArea">
                <input type="text" class="bocik-input" id="webhookInput" placeholder="Link do Webhooka...">
    ...
    // 3. Aktualizacja Wyglądu (Logika stanów)
    function updateGUI(state, timeLeft = 0) {
        if (state === 'IDLE') {
            panel.classList.remove('panel-alarm');
            if(!panel.style.borderColor.includes('555')) panel.style.borderColor = '#b026ff'; // Reset tylko jak nie zablokowane
            updateStatusText();
    ...
    // --- MODUŁ PRZESUWANIA I KOTWICZENIA (DRAG & PIN) ---
    (function makeDraggable(element, handle) {
        ...
        function dragMouseDown(e) {
            if (isPinned) return;
            // Ignorujemy kliknięcia w inputy
        ...
        function elementDrag(e) {
            ...
            // Reset position fixed constraints
    ...
    function performHardLogout() {
        updateGUI('LOGOUT');
        console.warn("[Bocik] WYKONUJĘ TWARDE WYLOGOWANIE!");
        const logoutBtn = document.getElementById('logoutbut');
        if (logoutBtn) logoutBtn.click();
        setTimeout(() => {
            window.location.href = "https://margonem.pl/";
        }, 7000);
    }
    ...
    function startCountdown() {
        ...
        console.log(`[Bocik] Start odliczania: ${randomSeconds}s`);
    ...
    function stopCountdown() {
        if (countdownInterval) {
            console.log("[Bocik] Zagadka rozwiązana.");
            clearInterval(countdownInterval);
            countdownInterval = null;
            updateGUI('IDLE');
        }
    ...
    console.log("Bocik v5.2 Neon: Ready.");

    // --- ODBIORNIK SYGNAŁU Z HUBA ---
    // A. Odczytaj zapamiętany stan (domyślnie true, jeśli nie ma zapisu)
    const savedState = localStorage.getItem(STORAGE_VISIBLE_KEY);
    const shouldBeVisible = savedState === null ? true : (savedState === 'true');

    // B. Ustaw widoczność na starcie
    panel.style.display = shouldBeVisible ? 'flex' : 'none';

    // C. Nasłuchuj na przełączenie z Huba
