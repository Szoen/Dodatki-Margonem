// ==UserScript==
// @name         Zagadka discord v5.5 [FIXED DRAG]
// @namespace    http://tampermonkey.net/
// @version      5.5
// @description  Wykrywa zagadkę, Panel Neon UI, Naprawione przesuwanie (No conflict), Test Webhooka
...
(function() {
    ...
    style.innerHTML = `
        ...
        .btn-save:hover { box-shadow: 0 0 8px rgba(92, 184, 92, 0.4); }
        .btn-test { background: #3a3a1c; color: #ffff7a; border: 1px solid #b8b85c; }
        .btn-test:hover { box-shadow: 0 0 8px rgba(255, 255, 122, 0.4); }
    ...
    panel.innerHTML = `
        ...
        <div class="bocik-content">
            ...
            <button class="bocik-btn btn-settings" id="toggleConfig">⚙️ Ustawienia / Test</button>
            ...
            <div class="config-container" id="configArea">
                ...
                <div class="btn-row">
                    <button class="bocik-btn btn-save" id="saveWebhook" style="flex:2">💾 ZAPISZ</button>
                    <button class="bocik-btn btn-test" id="testWebhook" style="flex:1">🔔 TEST</button>
                </div>
            </div>
        </div>
    `;
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                if(response.status >= 200 && response.status < 300) {
                    ...
                    alert("❌ Błąd! Kod: " + response.status);
                }
            },
            onerror: function(err) {
                console.error(err);
                alert("❌ Błąd połączenia! Sprawdź konsolę (F12).");
            }
        });
    };
    ...
    function updateStatusText() {
        if (!savedWebhookUrl) {
            ...
            statusText.style.borderColor = '#ffcc00';
        } else {
    ...
    function updateGUI(state, timeLeft = 0) {
        if (state === 'IDLE') {
            ...
            if(!panel.classList.contains('panel-locked')) panel.style.borderColor = '#b026ff';
            updateStatusText();
    ...
    // --- POPRAWIONY MODUŁ PRZESUWANIA (addEventListener) ---
    (function makeDraggable(element, handle) {
        ...
        function dragMouseDown(e) {
            ...
            // ZMIANA: Używamy addEventListener zamiast przypisywania onmouseup/onmousemove
        ...
        function closeDragElement() {
            // ZMIANA: Usuwamy listenery bezpiecznie
