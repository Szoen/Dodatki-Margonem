// ==UserScript==
// @name         1 - Bocik e2 - Ozirus [Neon UI + Memory] (FIXED)
...
(function() {
    ...
    const STORAGE_VISIBLE_KEY = 'bocik_ozirus_gui_visible'; // Klucz do zapamiętywania widoczności
    ...
    style.innerHTML = `
        .bocik-panel {
            ...
            color: #fff;
            overflow: hidden;
            display: none; /* Domyślnie ukryte, JS zdecyduje */
    ...
    // Dodajemy unikalne ID do głównego diva, żeby style się nie myliły
    ...
    // --- DRAGGABLE & PIN MODULE (WERSJA POPRAWIONA - BEZ KONFLIKTÓW) ---
    (function makeDraggable(element, dragHandle) {
        ...
        function dragMouseDown(e) {
            ...
            // ZMIANA: Używamy addEventListener zamiast onmouseup/onmousemove
    ...
    window.addEventListener('load', function() {
        ...
        window.addEventListener('bocik:toggle-gui', function(e) {
            if (e.detail.id === ADDON_ID) {
                // Sprawdź czy jest ukryty
                const isHidden = (gui.style.display === 'none' || gui.style.display === '');

                if (isHidden) {
                    gui.style.display = 'flex';
                    localStorage.setItem(STORAGE_VISIBLE_KEY, 'true'); // Zapamiętaj: POKAZANY
                } else {
                    gui.style.display = 'none';
                    localStorage.setItem(STORAGE_VISIBLE_KEY, 'false'); // Zapamiętaj: UKRYTY
                }
            }
        });
